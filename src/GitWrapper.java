...
public class GitWrapper {
    ...
    public void addFileRecursive(File f) throws Exception {
        if (f.equals(new File("./git")) || f.equals(new File("./src")) || f.equals(new File("./.git"))
                || f.equals(new File("./README.md"))) // check: don't add dumb stuff
            return;
        ...
        if (f.listFiles() == null) // check: listfiles might be null for an empty dir, don't add it
            return;
    ...
    public String commit(String author, String message) {
        System.out.println(Tree.createTrees());
        return "";
    };
